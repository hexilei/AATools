...
{
	public static final String TEXT_PATH = "\\main-dev\\sys_config_data\\text";
	
	public final static String[] paths = {
		"F:\\AA7.2.0.X\\main-dev\\sys_config_data\\text\\",
		"F:\\AA7.2.6.X\\main-dev\\sys_config_data\\text\\",
		"F:\\AA7.3.0.X\\main-dev\\sys_config_data\\text\\",
		"F:\\AA7.3.1.X\\main-dev\\sys_config_data\\text\\",
		"F:\\AA7.3.2.X\\main-dev\\sys_config_data\\text\\",
		"F:\\AA7.3.3.X\\main-dev\\sys_config_data\\text\\",
	};
...
//	}
	...
	public static List<String> retrieveInfo(String strKey, String rootPath) throws Exception {
		List<String> infoList = new ArrayList<String>();
		String tempValue = "";
		File f = new File(rootPath + "\\main-dev\\sys_config_data\\text\\" + files[0]);
		if (!f.exists()) {
			return infoList;
		}
	...
	public static boolean updateText(String strKey, String strValue, String category, String recDate, String recName,
			boolean popCommit, String path, boolean flag) throws Exception {
		// update files by using svn command
		if (popCommit) {
			updateBySVN(path + "\\main-dev\\sys_config_data\\text\\");
		}
		...
		for (int i = 0; i < files.length; i++) {
			...
			flag = updateToFile(strKey, strValue, category, recDate, recName, files[i], path, flag);
			strValue = orgValue;
		}

		// commit files by using svn command
		if (popCommit && flag) {
			commitBySVN(path + "\\main-dev\\sys_config_data\\text\\");
		}
		return flag;
	}
	
	public static boolean updateToFile(String key, String value, String category, String recDate, String recName,
			String fn, String path, boolean flag) throws Exception {
		File f = new File(path + "\\main-dev\\sys_config_data\\text\\" + fn);
		if (!f.exists()) {
			return false;
		}
		InputStream in = new FileInputStream(f);
	...
	public static boolean addText(String strKey, String strValue, String category, String recDate, String recName,
			String recStatus, boolean popCommit, String targetPath) throws Exception {
		// update files by using svn command
		if (popCommit) {
			updateBySVN(targetPath + "\\main-dev\\sys_config_data\\text\\");
		}

		for (int i = 0; i < files.length; i++) {
			StringBuilder builder = new StringBuilder();
			// workflow.email.at.label,"at_ÖÐÎÄ",Portlet - Process
			// Email,11/19/2010,ADMIN,A,CN,zh
			...
			addToFile(builder.toString(), files[i], targetPath + "\\main-dev\\sys_config_data\\text\\");
		}

		// commit files by using svn command
		if (popCommit) {

			commitBySVN(targetPath + "\\main-dev\\sys_config_data\\text\\");
	...
	public static void addToFile(String v, String fn, String targetPath) throws Exception {
		File f = new File(targetPath + "\\main-dev\\sys_config_data\\text\\" + fn);
		if (!f.exists()) {
			return;
		}
		InputStream in = new FileInputStream(f);
