 ...
 * </pre>
 */
public class PrepareProxyThread extends Thread
{
	...
	public PrepareProxyThread(Socket client) throws Exception
	{
		this.client = client;
		...
		ConnectionUtil.sockets.add(server);
	...
	public void run()
	{
		try
		{
			int count = ConnectionUtil.getCurrentResponses().size();
			if (dbType.equals("ORACLE")) {
				...
				// write to db server
				int headSize = CollectorProxyThread.HEAD_SIZE;
				if ("7.1.0".equals(ver)) {
					headSize = CollectorProxyThread.HEAD_SIZE_710;
				}
				...
				str = str.replace(Integer.toString(DBProxy.PRE_PORT), port);
				body = str.getBytes();
				...
				Debug.println("Pre Client -> Server: " + new String(head) + new String(body));
				count -= 1;
			}
			
			new PreparePiper("Pre Server -> Client", sin, cout, ConnectionUtil.getCurrentResponses().size()).start();
			new PreparePiper("Pre Client -> Server", cin, sout, count).start();
		}
		catch (Exception e1)
		{
			e1.printStackTrace();
